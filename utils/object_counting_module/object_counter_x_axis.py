...
def count_objects_x_axis(top, bottom, right, left, crop_img, roi_position, y_min, y_max, deviation):   
        ...
        if (abs(((right+left)/2)-roi_position) < deviation):
          ...
          print("Passei pela condição de centro do box na rotina count_objects_x_axis")
        else:
          if (abs(((right+left)/2)-roi_position)) >=deviation and (abs(((right+left)/2)-roi_position)) <=deviation+1:
             print("Distancia do centro do box",(abs(((right+left)/2)-roi_position)), "Botton:", bottom, "bottom referencia:",bottom_position_of_detected_vehicle[0])
